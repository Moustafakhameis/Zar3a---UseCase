// Actor visual theme — strong visible colors for both light AND dark mode.

export const ACTOR_COLORS = {
  admin: {
    color: 'text-rose-600 dark:text-rose-400',
    bg: 'bg-rose-50 dark:bg-rose-900/40',
    border: 'border-rose-300 dark:border-rose-500',
    hoverBg: 'hover:bg-rose-100 dark:hover:bg-rose-800/50',
    lineColor: '#f43f5e',
    lineColorDim: 'rgba(244,63,94,0.12)',
    dotBg: 'bg-rose-500',
    gradient: 'from-rose-500 to-rose-600',
  },
  supplier: {
    color: 'text-amber-600 dark:text-amber-400',
    bg: 'bg-amber-50 dark:bg-amber-900/40',
    border: 'border-amber-300 dark:border-amber-500',
    hoverBg: 'hover:bg-amber-100 dark:hover:bg-amber-800/50',
    lineColor: '#f59e0b',
    lineColorDim: 'rgba(245,158,11,0.12)',
    dotBg: 'bg-amber-500',
    gradient: 'from-amber-500 to-amber-600',
  },
  farmer: {
    color: 'text-emerald-600 dark:text-emerald-400',
    bg: 'bg-emerald-50 dark:bg-emerald-900/40',
    border: 'border-emerald-300 dark:border-emerald-500',
    hoverBg: 'hover:bg-emerald-100 dark:hover:bg-emerald-800/50',
    lineColor: '#10b981',
    lineColorDim: 'rgba(16,185,129,0.12)',
    dotBg: 'bg-emerald-500',
    gradient: 'from-emerald-500 to-emerald-600',
  },
  buyer: {
    color: 'text-sky-600 dark:text-sky-400',
    bg: 'bg-sky-50 dark:bg-sky-900/40',
    border: 'border-sky-300 dark:border-sky-500',
    hoverBg: 'hover:bg-sky-100 dark:hover:bg-sky-800/50',
    lineColor: '#0ea5e9',
    lineColorDim: 'rgba(14,165,233,0.12)',
    dotBg: 'bg-sky-500',
    gradient: 'from-sky-500 to-sky-600',
  },
  specialist: {
    color: 'text-violet-600 dark:text-violet-400',
    bg: 'bg-violet-50 dark:bg-violet-900/40',
    border: 'border-violet-300 dark:border-violet-500',
    hoverBg: 'hover:bg-violet-100 dark:hover:bg-violet-800/50',
    lineColor: '#8b5cf6',
    lineColorDim: 'rgba(139,92,246,0.12)',
    dotBg: 'bg-violet-500',
    gradient: 'from-violet-500 to-violet-600',
  },
};
